...
int main()
{
    ...
    int channel = 3;
    std::vector<float> image(image_width * image_height * channel, 0.0);
    glReadPixels(0, 0, image_width, image_height, GL_RGB, GL_FLOAT, image.data());
    
    std::vector<unsigned char> image_(image.size(), 0);
    for (int i = 0; i < image.size(); i++)
    {
        image_[i] = static_cast<unsigned char>(image[i] * 255);
    }
    
    std::string image_path = "/Users/yubanxian/data/lut.png";
    stbi_flip_vertically_on_write(true);
    stbi_write_png(image_path.c_str(), image_width, image_height, channel, image_.data(), 0);
